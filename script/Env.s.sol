...
contract DeployBurve is Script {
    ...
    function getTokenSets() internal pure returns (TokenSet[] memory) {
        ...
        // BTC Set
        TokenConfig[] memory btcTokens = new TokenConfig[](10);
        ...
        bgtTokens[0] = TokenConfig("Wrapped Bera", "WBERA", 18);
        bgtTokens[1] = TokenConfig("Infrared BGT", "iBGT", 18);
        bgtTokens[2] = TokenConfig("stBGT", "stBGT", 18);

        TokenSet[] memory sets = new TokenSet[](4);
        sets[0] = TokenSet("USD", usdTokens);
        sets[1] = TokenSet("BTC", btcTokens);
        sets[2] = TokenSet("ETH", ethTokens);
        sets[3] = TokenSet("BGT", bgtTokens);
