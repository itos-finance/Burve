#!/usr/bin/env python3
"""
Script to calculate BERA distribution amounts based on points.
"""
...
TOTAL_DISTRIBUTION = 100_000  # 100k BERA

def main():
    # Calculate total points
    total_points = sum(points for _, points in recipients)
    print(f"Total points: {total_points:,.2f}")
    print(f"Total distribution: {TOTAL_DISTRIBUTION:,} BERA\n")

    # Calculate each recipient's share
    print("Address,Points,Percentage,BERA Amount")
    print("-" * 80)

    total_distributed = 0

    for address, points in recipients:
        percentage = (points / total_points) * 100
        bera_amount = (points / total_points) * TOTAL_DISTRIBUTION
        total_distributed += bera_amount

        # Format for Solidity (checksummed address format)
        checksum_addr = address  # You can use web3.py for proper checksumming if needed

        print(f"{checksum_addr},{points:,.2f},{percentage:.4f}%,{bera_amount:.18f} BERA")

    print("-" * 80)
    print(f"Total distributed: {total_distributed:,.18f} BERA")
    print(f"Difference from target: {abs(TOTAL_DISTRIBUTION - total_distributed):.18f} BERA")

    # Generate Solidity code
    print("\n\n// Solidity Recipients Array:")
    print("// Copy this into your constructor:")
    for address, points in recipients:
        percentage = (points / total_points)
        bera_amount = percentage * TOTAL_DISTRIBUTION
        # Convert to checksum format
        parts = address[2:].lower()
        checksum_addr = f"0x{parts[0].upper()}{parts[1:]}"
        print(f"recipients.push(Recipient({checksum_addr}, {bera_amount:.18f} ether));")
