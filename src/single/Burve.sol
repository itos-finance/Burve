...
import { TickRange, TickRangeImpl } from "./TickRange.sol";
...
contract Burve is ERC20 {
    ...
    uint128 private totalLiqNominal;

    mapping(address owner => uint256 islandShares) islandSharesPerOwner;
    ...
    function mint(
        ...
        uint256 shares;
        if (totalLiqNominal == 0) {
            shares = mintLiqNominal;
        } else {
            shares = FullMath.mulDiv(mintLiqNominal, totalSupply(), totalLiqNominal);
        }
    ...
    function burn(
        ...
        uint128 burnLiqNominal = uint128(FullMath.mulDiv(shares, uint256(totalLiqNominal), totalSupply()));
        ...
        for (uint256 i = 0; i < distX96.length; ++i) {
            ...
            if (range.isIsland()) {
                ...
                uint128 liqInRange = uint128(shift96(burnLiqNominal * distX96[i], true));
        ...
        TransferHelper.safeTransferFrom(address(token0), address(this), msg.sender, postBalance0 - priorBalance0);
        TransferHelper.safeTransferFrom(address(token1), address(this), msg.sender, postBalance1 - priorBalance1);
    }
    ...
    function compoundV3Ranges() internal {
        ...
        uint256 amount0InUnitLiq;
        uint256 amount1InUnitLiq;
        ...
        for (uint256 i = 0; i < distX96.length; ++i) {
            ...
            (uint256 range0, uint256 range1) = getAmountsForLiquidity(
                unitLiqNominalX64,
                range.lower,
                range.upper
            );
            amount0InUnitLiq += range0;
            amount1InUnitLiq += range1;
        ...
        uint128 liqNominal0X64 = uint128((collected0 / amount0InUnitLiq) >> 64);
        uint128 liqNominal1X64 = uint128((collected1 / amount1InUnitLiq) >> 64);

        uint128 compoundedLiqNominal = liqNominal0X64 < liqNominal1X64 ? liqNominal0X64 : liqNominal1X64;
        ...
        for (uint256 i = 0; i < distX96.length; ++i) {
            ...
            uint128 liqInRange = uint128(shift96(compoundedLiqNominal * distX96[i], true));
