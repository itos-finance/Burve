...
contract UniswapV3Pool is IUniswapV3Pool, NoDelegateCall {
    ...
    event UpdateDynamicFeeRate(uint24 dynamicFee);
    ...
    /// @notice a dynamic fee rate can be set on a per pool basis by the owner
    /// @dev initialized to the fee rate set in the pool deployer as INIT_FEE
    uint24 dynamicFee;
        ...
        returns (int256 amount0, int256 amount1)
    {
        ...
        while (
            ...
            (
                ...
                /// @notice the fee rate has been replaced by a dynamic rate set on each pool
                dynamicFee
            );
    ...
    /// @notice the Burve pools do not have a set fee rate, rather it is set
    /// dynamically by the factory owner
    /// @param _dynamicFee expressed in hundredths of a bip
    function setDynamicFee(
        uint24 _dynamicFee
    ) external override onlyFactoryOwner {
        dynamicFee = _dynamicFee;

        emit UpdateDynamicFeeRate(_dynamicFee);
    }
}
